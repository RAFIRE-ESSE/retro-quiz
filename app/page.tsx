...
export default function HomePage() {
  ...
  return (
    <div>
      ...
      <section className="retro-card" style={{ padding: '2.5rem 1.5rem', textAlign: 'center', marginBottom: '2.5rem', backgroundColor: 'var(--color-cream)' }}>
        <div style={{ display: 'inline-block', transform: 'rotate(-2deg)', marginBottom: '1rem' }}>
          <span className="retro-sticker sticker-plum">★ 100% RETRO ARCADE TRIVIA ★</span>
        </div>

        <h1 style={{ fontSize: 'clamp(2.2rem, 5vw, 3.4rem)', fontWeight: 800, color: 'var(--color-plum)', lineHeight: 1.1, marginBottom: '0.75rem', letterSpacing: '-0.02em' }}>
          INSERT COIN &amp; PLAY
        </h1>

        <p style={{ fontSize: '1.2rem', color: 'var(--color-secondary)', maxWidth: '650px', margin: '0 auto 1.75rem', fontWeight: 600 }}>
        ...
        {/* Gamer Tag Handle Input */}
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.75rem', backgroundColor: 'var(--card-bg)', border: '3px solid var(--border-main)', padding: '0.6rem 1.25rem', borderRadius: '8px', boxShadow: 'var(--shadow-retro-sm)' }}>
          <RetroCoinIcon size={26} />
          <span className="font-arcade" style={{ fontSize: '1.35rem', color: 'var(--color-plum)' }}>
            GAMER TAG:
          </span>
          <input
            type="text"
      ...
      <section style={{ marginBottom: '2.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--color-plum)' }}>
            1. SELECT GAME MODE
          </h2>
          <span className="font-arcade" style={{ fontSize: '1.2rem', color: 'var(--color-rose)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
          ...
          <div
            ...
            style={{
              padding: '1.25rem',
              cursor: 'pointer',
              backgroundColor: gameMode === 'standard' ? 'var(--color-teal)' : 'var(--card-bg)',
          ...
          >
            ...
            <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: 'var(--color-plum)', marginBottom: '0.35rem' }}>
              Standard Arcade
            </h3>
            <p style={{ fontSize: '0.88rem', color: gameMode === 'standard' ? '#0d2820' : 'var(--text-secondary)' }}>
          ...
          <div
            ...
            style={{
              padding: '1.25rem',
              cursor: 'pointer',
              backgroundColor: gameMode === 'blitz' ? 'var(--color-rose)' : 'var(--card-bg)',
              borderColor: 'var(--color-plum)',
              color: gameMode === 'blitz' ? '#fff' : 'inherit'
            }}
          >
            <div style={{ marginBottom: '0.65rem' }}>
              <RetroLightningIcon size={36} />
            </div>
            <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: gameMode === 'blitz' ? '#fff' : 'var(--color-plum)', marginBottom: '0.35rem' }}>
              Time Attack Blitz
            </h3>
            <p style={{ fontSize: '0.88rem', color: gameMode === 'blitz' ? '#ffe5ed' : 'var(--text-secondary)' }}>
          ...
          <div
            ...
            style={{
              padding: '1.25rem',
              cursor: 'pointer',
              backgroundColor: gameMode === 'practice' ? '#d0e8b0' : 'var(--card-bg)',
              borderColor: 'var(--color-plum)'
            }}
          >
            <div style={{ marginBottom: '0.65rem' }}>
          ...
          <div
            ...
            style={{
              padding: '1.25rem',
              cursor: 'pointer',
              backgroundColor: gameMode === 'survival' ? '#fecaca' : 'var(--card-bg)',
              borderColor: 'var(--color-plum)'
            }}
          >
            <div style={{ marginBottom: '0.65rem' }}>
            ...
            <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: '#991b1b', marginBottom: '0.35rem' }}>
              Sudden Death
            </h3>
            <p style={{ fontSize: '0.88rem', color: 'var(--text-secondary)' }}>
      ...
      <section>
        ...
        {loading ? (
          <div className="retro-card" style={{ padding: '3rem', textAlign: 'center' }}>
            <div className="font-arcade" style={{ fontSize: '2rem', color: 'var(--color-plum)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
            {quizzes.map((quiz) => (
              <div key={quiz.id} className="retro-card retro-card-interactive" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1rem' }}>
                    <div style={{ width: '56px', height: '56px', backgroundColor: 'var(--color-cream)', border: '2.5px solid var(--border-main)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '2px 2px 0px var(--shadow-color)' }}>
                      {renderCartridgeIcon(quiz.slug, quiz.category)}
                    </div>
                    <span className="retro-sticker sticker-teal" style={{ fontSize: '0.9rem' }}>
                      {quiz.questionCount} QUESTIONS
                    </span>
                  </div>

                  <h3 style={{ fontSize: '1.3rem', fontWeight: 800, color: 'var(--color-plum)', marginBottom: '0.5rem' }}>
                    {quiz.title}
                  </h3>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '2px dashed var(--color-rose)', paddingTop: '1rem' }}>
                  <span className="font-arcade" style={{ fontSize: '1.15rem', color: 'var(--color-rose)' }}>
